...
# Original source: https://github.com/huggingface/transformers/blob/2e24ee4dfa39cc0bc264b89edbccc373c8337086/src/transformers/models/whisper/generation_whisper.py
# Modified by: Dominik Wagner, TH Nürnberg
# Modifications: Ensure that custom arguments for the optimized version of speculative sampling are passed to
# the generate method.
...
class CustomWhisperGenerationMixin(CustomGenerationMixin):
    def _extract_token_timestamps(
        self, generate_outputs, alignment_heads, time_precision=0.02, num_frames=None
    ):
        """
        ...
        if num_frames is not None:
            ...
            if len(np.unique(num_frames)) == 1:
                ...
                num_frames = (
                    num_frames if isinstance(num_frames, int) else num_frames[0]
                )
        ...
        for batch_idx in range(batch_size):
            if num_frames is not None and isinstance(
                num_frames, (tuple, list, np.ndarray)
            ):
    ...
    def generate_with_fallback(
        ...
        for fallback_idx, temperature in enumerate(temperatures):
            ...
            generation_config.num_beams = (
                kwargs.pop("num_beams", 1) if not generation_config.do_sample else 1
            )
    ...
    def _retrieve_init_tokens(
        ...
        if forced_decoder_ids is not None and forced_decoder_ids[0][0] == 1:
            ...
            generation_config.forced_decoder_ids = (
                forced_decoder_ids if len(forced_decoder_ids) > 0 else None
            )
    ...
    def _set_token_ids(generation_config, config, kwargs):
        ...
        generation_config.eos_token_id = (
            eos_token_id if eos_token_id is not None else config.eos_token_id
        )
        generation_config.decoder_start_token_id = (
            decoder_start_token_id
            if decoder_start_token_id is not None
    ...
    def _retrieve_logit_processors(
        ...
        if is_shortform and generation_config.forced_decoder_ids is not None:
            forced_tokens_proc = ForceTokensLogitsProcessor(
                generation_config.forced_decoder_ids
            )
    ...
    def _set_max_new_tokens_and_length(
        config, decoder_input_ids, generation_config, kwargs
    ):
        ...
        if passed_max_length is not None and passed_max_new_tokens is None:
            ...
            max_length_config is not None
            and passed_max_new_tokens is None
            and max_new_tokens_config is None
        ):
            max_length = min(
            ...
            and passed_max_new_tokens + decoder_input_ids.shape[-1]
            > config.max_target_positions
        ):
            ...
            and max_new_tokens_config + decoder_input_ids.shape[-1]
            > config.max_target_positions
        ):
            max_new_tokens = config.max_target_positions - decoder_input_ids.shape[-1]
