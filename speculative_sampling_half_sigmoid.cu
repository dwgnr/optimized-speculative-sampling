...
__global__ void sampling_kernel_sigmoid(at::Half* p, at::Half* q, at::Half* probability_ratio, at::Half* p_prime, at::Half* clamped_sum, int batch_size,
    ...
    if (ix < batch_size && iy < tokens) {
        for (int tile_start = 0; tile_start < vocab_size; tile_start += threads) {
            ...
            if (current_idx < tile_size) {
                at::Half elem_p = p[idx * vocab_size + tile_start + current_idx];
                at::Half elem_q = q[idx * vocab_size + tile_start + current_idx];

                if (elem_p < MIN_LOGIT_VAL) {
                    p_tile[current_idx] = at::Half(0.0f);
                } else if (elem_p > MAX_LOGIT_VAL) {
                    p_tile[current_idx] = at::Half(1.0f);
                }
                else {
                ...
                if (elem_q < MIN_LOGIT_VAL) {
                    q_tile[current_idx] = at::Half(0.0f);
                } else if (elem_q > MAX_LOGIT_VAL) {
                    q_tile[current_idx] = at::Half(1.0f);
                } else {
...
sampling_cuda(torch::Tensor candidate_input_ids,
    ...
    sampling_kernel_sigmoid<<<grid_size_div, block_size_div, sram_size_div>>>(
            new_logits.data_ptr<at::Half>(),
            candidate_logits.data_ptr<at::Half>(),
